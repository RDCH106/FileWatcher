	...
	if( nOld == nNew )
	{
		...
		while( oldListPos != NULL && newListPos != NULL )
		{
			...
			if( *oldFI != *newFI )
			{
				...
				if(*oldFI->m_fd.cFileName == *newFI->m_fd.cFileName){ //Case 1: Name not modificated

					fi     = *newFI;
					faType = faChange;
					break;

				}else{ //Case 2: Name modificated

					int *list_mask = new int[newList->GetCount()]; // Generate an array mask to find the renamed file
					for(int i=0; i<oldList->GetCount(); i++){ //Intialize array mask
						list_mask[i] = 0;
					}

					for(int i=0; i<newList->GetCount(); i++){ // Search file

						for(int j=0; j<oldList->GetCount(); j++){

							if(*newList->GetAt(newList->FindIndex(i)) == *oldList->GetAt(oldList->FindIndex(j))){

								list_mask[i] = -1; //If not renamed position value in the mask is -1
								break;
							}
						}
					}
					...
					bool found = false;
					int index = 0;
					while(!found){ //Find the unique 0 value to get position (only be one)
						if(list_mask[index] == 0){
							
							fi     = *(newList->GetAt(newList->FindIndex(index))); //Get the file from newList at searched position
							faType = faChange;
							break;
						}
						index++;
...
void CFileInformation::RemoveDir( CString dir )
{
	...
	strcpy( strFindFiles, OLE2A(dir.GetBuffer( dir.GetLength() )) );
    strcat( strFindFiles, "\\*.*" );
    
	hndle = FindFirstFile( A2OLE(strFindFiles), &find );
    ...
    {    
		...
		strcpy( strFolderItem, OLE2A(dir.GetBuffer( dir.GetLength() )) );
        strcat( strFolderItem, "\\" );
		strcat( strFolderItem, OLE2A(find.cFileName));
		
		if( ( !strcmp( OLE2A(find.cFileName), "."  ) ) || 
			( !strcmp( OLE2A(find.cFileName), ".." ) ) ) 
        {
            RemoveDirectory( A2OLE(strFolderItem) );
		...
		{
            SetFileAttributes( A2OLE(strFolderItem), FILE_ATTRIBUTE_NORMAL );
            DeleteFile( A2OLE(strFolderItem) );
		}
...
CString CFileInformation::GenerateNewFileName(CString path)
{
	...
	FILE*   pFile   = fopen( CT2A(newPath), "r" );
